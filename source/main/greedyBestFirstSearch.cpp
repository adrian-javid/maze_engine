...
auto Project::greedyBestFirstSearch(
    ...
    */
    auto const heuristicComparator = [&maze, &end](Vector2 const &leftKey, Vector2 const &rightKey) constexpr -> bool {
        return maze.length(leftKey - end) > maze.length(rightKey - end);
    };

    return simpleSearch<
        std::priority_queue<Vector2, std::vector<Vector2>, decltype(heuristicComparator)>
    >(maze, start, processKey);
}
